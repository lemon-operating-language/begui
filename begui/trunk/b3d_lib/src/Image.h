...
Color Image::color(size_t x, size_t y) const
{
	...
	switch (m_format) {
		case Image::I8BITS:
			cl.r = at<uint8_t>(x,y,0) / 255.0;
			if (m_nChannels > 1)
				cl.g = at<uint8_t>(x,y,1) / 255.0;
			if (m_nChannels > 2)
				cl.b = at<uint8_t>(x,y,2) / 255.0;
			break;
		case Image::I16BITS:
			cl.r = at<uint16_t>(x,y,0) / 255.0;
			if (m_nChannels > 1)
				cl.g = at<uint16_t>(x,y,1) / 255.0;
			if (m_nChannels > 2)
				cl.b = at<uint16_t>(x,y,2) / 255.0;
			break;
		case Image::I32BITS:
			cl.r = at<uint32_t>(x,y,0) / 255.0;
			if (m_nChannels > 1)
				cl.g = at<uint32_t>(x,y,1) / 255.0;
			if (m_nChannels > 2)
				cl.b = at<uint32_t>(x,y,2) / 255.0;
			break;
		case Image::F16BITS:
			Console::error("Image::color() : F16BITS not supported!!\n");
			break;
		case Image::F32BITS:
			cl.r = at<float32_t>(x,y,0);
			if (m_nChannels > 1)
				cl.g = at<float32_t>(x,y,1);
			if (m_nChannels > 2)
				cl.b = at<float32_t>(x,y,2);
			break;
		case Image::F64BITS:
			cl.r = at<float64_t>(x,y,0);
			if (m_nChannels > 1)
				cl.g = at<float64_t>(x,y,1);
			if (m_nChannels > 2)
				cl.b = at<float64_t>(x,y,2);
			break;
	}
	return cl;
}
