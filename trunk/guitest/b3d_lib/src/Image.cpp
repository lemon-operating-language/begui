...
/*void Image::toTexture(Texture *pTex)
{
	ASSERT(pTex);
	...
	// convert w and h to powers of 2
	while (w<m_width)
		w *= 2;
	while (h<m_height)
		h *= 2;
	...
	for (int i=0; i<m_width && i<2048; ++i)
		for (int j=0; j<m_height && j<2048; ++j)
		{
			if (m_nChannels == 1)
			{
				data[3*(j*w + i)] = m_data[j*m_width + i];
				data[3*(j*w + i)+1] = m_data[j*m_width + i];
				data[3*(j*w + i)+2] = m_data[j*m_width + i];
			}
			else if (m_nChannels >= 3)
			{
				data[3*(j*w + i)] = m_data[3*(j*m_width + i)];
				data[3*(j*w + i)+1] = m_data[3*(j*m_width + i)+1];
				data[3*(j*w + i)+2] = m_data[3*(j*m_width + i)+2];
			}
			else
			{
				ASSERT(m_nChannels != 2);
				Console::error("Image::toTexture: incombatible types\n");
				return;
			}
		}
	pTex->create(w, h, GL_RGB, data);
}*/
...
/*	// find the tangent at pixel (x,y)
	...
	double tmp_i;
	double dfx = modf(fx, &tmp_i);
	double dfy = modf(fy, &tmp_i);
...
bool Image::loadBMP(const std::string &fname)
{
	...
	AUX_RGBImageRec* pImg = auxDIBImageLoad(fname.c_str());
	if (pImg == 0) {
		Console::error("could not load texture file %s\n", fname.c_str());
		return false;
	}
	...
	// copy data flipping vertically
	for (size_t i=0; i<m_width; ++i)
		for (size_t j=0; j<m_height; ++j)
		{
			size_t id1 = j*m_width+i;
			size_t id2 = (m_height-1-j)*m_width+i;
			m_data[3*id1]	= pImg->data[3*id2];
			m_data[3*id1+1]	= pImg->data[3*id2+1];
			m_data[3*id1+2]	= pImg->data[3*id2+2];
		}
...
bool Image::load(const std::string &fname)
{
	...
	else if (stricmp(ext.c_str(), "ppm") == 0)
		...
		for (size_t y=0; y<m_height; ++y)
		{
			for (size_t c=0; c<m_nChannels; ++c)
			{
				...
				switch (format) {
					...
					case Image::F16BITS: throw std::exception("format not supported"); break;
					case Image::F32BITS: newimg.at<float32_t>(x,y,c) = val; break;
					case Image::F64BITS: newimg.at<float64_t>(x,y,c) = val; break;
